...
public class GoogleMapsDemo : MonoBehaviour
{
    ...
    public static Color StaticColor { get; private set; }
...
#pragma warning restore 0649
	...
	List<PlusCodeInfo> infoList = new List<PlusCodeInfo>();
	...
	public void setOverLaySimple(float loclatf, float loclonf, float nsDistance, float ewDistance, int available, string code)
	{
		GroundOverlay groundOverlay = _map.AddGroundOverlay(CreateInitialGroundOverlayAvailable(loclatf,loclonf,ewDistance,nsDistance, available));
		//nameDictionary.Add(groundOverlay.Id, code);
		PlusCodeInfo pci = new PlusCodeInfo();
		pci.Id = groundOverlay.Id;
		pci.PlusCodeId = code;
		pci.available = available;
		infoList.Add(pci);
	}
	...
	public class PlusCodeInfo
	{
		 ...
		 public string PlusCodeId {get;set;}
		 public int available {get;set;}
	}

	public void setPlusCodesColor(float nelatf, float nelonf, float swlatf, float swlonf, float loclatf, float loclonf, Color c)
	{
		LatLng loc = new LatLng(loclatf,loclonf);
		...
		var polyline = _map.AddPolyline(CreateInitialPolylineColor(A,B,C,D,c));
	}
	...
	//		.Image(ImageDescriptor.FromAsset("overlay.png")) // image must be in StreamingAssets folder!
			.Image(ImageDescriptor.FromAsset("diamond.png"))
			.Anchor(0.5f, 0.5f)
			.Bearing(0)
			.Clickable(true)
			.Transparency(0.3f)
	...
	GroundOverlayOptions CreateInitialGroundOverlayAvailable(float loclatf, float loclonf, float ewDistance, float nsDistance, int available)
	{
		if(available == 0)
		{
			return new GroundOverlayOptions()
				.Position(new LatLng(loclatf, loclonf), ewDistance, nsDistance)
				.Image(ImageDescriptor.FromAsset("overlayredline.png")) // image must be in StreamingAssets folder!
				.Anchor(0.5f, 0.5f)
				.Bearing(0)
				.Clickable(true)
				.Transparency(0.2f)
		...
		else if(available == 1)
		{
			return new GroundOverlayOptions()
				.Position(new LatLng(loclatf, loclonf), ewDistance, nsDistance)
				.Image(ImageDescriptor.FromAsset("diamond.png")) // image must be in StreamingAssets folder!
				.Anchor(0.5f, 0.5f)
				.Bearing(0)
				.Clickable(true)
				.Transparency(0.2f)
		...
		else if(available == 2)
		{
			return new GroundOverlayOptions()
				.Position(new LatLng(loclatf, loclonf), ewDistance, nsDistance)
				.Image(ImageDescriptor.FromAsset("overlayyellowline.png")) // image must be in StreamingAssets folder!
				.Anchor(0.5f, 0.5f)
				.Bearing(0)
				.Clickable(true)
				.Transparency(0.2f)
		...
		else
		{
			return new GroundOverlayOptions()
				.Position(new LatLng(loclatf, loclonf), ewDistance, nsDistance)
				.Image(ImageDescriptor.FromAsset("overlaygreenline.png")) // image must be in StreamingAssets folder!
				.Anchor(0.5f, 0.5f)
				.Bearing(0)
				.Clickable(true)
				.Transparency(0.3f)
	...
	public static PolylineOptions CreateInitialPolylineColor(LatLng A, LatLng B, LatLng C, LatLng D, Color c)
	{
	    return new PolylineOptions()
	        .Add(A, B, C, D, A)
	        .Clickable(false)
	...
	public void addMapPinLatLng(LatLng point)
  {
	...
	void OnMapReady(GoogleMapsView googleMapsView)
	{
		...
		//_map.SetOnGroundOverlayClickListener(overlay => onOverlayClicked(overlay, overlay.Id));
	...
	GroundOverlay selectoverlay;
	string imgd;

	void onOverlayClicked(GroundOverlay a, string id)
	{
		Debug.Log("Ground overlay clicked: " + a);

		foreach(PlusCodeInfo info in infoList)
		{
			if (info.Id == id)
			{
					if(selectoverlay != null)
					{
						selectoverlay.SetImage(ImageDescriptor.FromAsset(imgd));
					}
					Debug.Log(info.PlusCodeId + ":" + info.available);
					kml.kmlUI(info.PlusCodeId, info.available);
					selectoverlay = a;
					imgd = images[info.available];
					a.SetImage(ImageDescriptor.FromAsset(selectimages[info.available]));
					Debug.Log(selectimages[info.available]);
			}
		}

		GroundOverlayOptions CreateOverlayReplace(LatLngBounds thebounds)
		{
			return new GroundOverlayOptions()
				.PositionFromBounds(thebounds)
				.Image(ImageDescriptor.FromAsset("overlay.png")) // image must be in StreamingAssets folder!
				.Anchor(0.5f, 0.5f)
				.Bearing(0)
				.Clickable(false)
...
/*
		...
		{
				// Retrieve and return the name associated with the ID
				Debug.Log(nameDictionary[id]);
		}
		else
		{
				Debug.LogError("ID not found in the dictionary!");
		}
*/
	}
